...
void    TestVoicePlay(void)
{
    ...
    _VOICE_ACT = VOICE_OFF; // Î¶¨ÏÖã ?òÏù¥ : RFID ?ïÏÉÅ ?ôÏûë 
    ...
    for (;1;)
    {
		...
		if (status == MI_OK)    // MIFARE Ïπ¥Îìú?ºÎïåÎß??ëÎèô
		{
			//mytest = str[0];
			//mytest1 = str[1];
		}	

		// RFID Ï∂©ÎèåÎ∞©Ï?, RFID ?úÍ∑∏??IDÍ∞??úÎ¶¨?ºÎÑòÎ≤? ???Ä?•Îêú Í∞íÏùÑ Î¶¨ÌÑ¥?? 4Byte
		...
		AddicoreRFID_Halt();  // ?ôÏûë Ï§ëÏ? ?úÌÇ§??Í±¥Îç∞ ?ôÏûë ?àÌïò?îÍ±∞ Í∞ôÎã§. 
    }
